...
export const SidebarAdmin = () => {
  ...
  const renderListItem = (item) => (
    <StyledListItem
      ...
      onClick={() =>
        item.id === "logout" ? handleLogout() : navigate(`/admin/${item.id}`)
      }
    >
      <StyledListItemIcon>
  ...
  return (
    <MainContainer>
      ...
      <ListItem
        button
        style={{ paddingLeft: "15%", marginBottom: "16px" }}
        onClick={handleLogout}
      >
        <Divider style={{ backgroundColor: "floralwhite" }} />
        <StyledListItemIcon>
          <ExitToAppOutlinedIcon
            style={{ color: "floralwhite", fontSize: "1.25rem" }}
          />
        </StyledListItemIcon>
