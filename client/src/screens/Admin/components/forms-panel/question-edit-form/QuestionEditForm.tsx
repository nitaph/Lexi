...
const QuestionEditForm: React.FC<QuestionEditFormProps> = ({
  ...
  const handleTypeChange = useCallback(
    (e) => {
      ...
      setValue(
        `questions.${selectedQuestionIndex}.props`,
        defaultQuestionProps[newType],
        {
          shouldValidate: true,
        },
      );
  ...
  const renderInputField = (register, selectedQuestionIndex, key, value) => {
    if (key === "selectionOptions") {
      ...
      const fieldError =
        errors?.questions?.[selectedQuestionIndex]?.props?.[key];

      return (
        <TextField
          label={
            key === "fieldKey" ? "key" : key === "label" ? "Question Text" : key
          }
          ...
          {...register(`questions.${selectedQuestionIndex}.props.${key}`, {
            ...
            validate:
              key === "fieldKey"
                ? (value) =>
                    isCamelCase(value) ||
                    "Field Key must be in camelCase format"
                : null,
          })}
          onChange={(e) => {
            setValue(
  ...
  const renderInputsForSelectedType = (
    type,
    register,
    selectedQuestionIndex,
  ) => {
    ...
    const questionProps =
      watchedForm.questions[selectedQuestionIndex].props ||
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      <TextField
        ...
        helperText={getFormErrorMessage(
          errors[`questions.${selectedQuestionIndex}.type`],
        )}
        required
        fullWidth
        {...register(`questions.${selectedQuestionIndex}.type`, {
          required: "required",
        })}
