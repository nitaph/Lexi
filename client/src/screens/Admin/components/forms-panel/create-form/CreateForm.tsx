...
import {
  ...
  saveForm,
  updateForm,
} from "../../../../../DAL/server-requests/forms";
import Question, {
...
export const CreateForm = ({ editFormId, setEditFormId, setForms }) => {
  ...
  const addQuestion = useCallback(async () => {
    if (questionLength >= 50) {
      openSnackbar(
        `Form Qustions Limit Exceeded: ${questionLength}`,
    ...
    const response = await trigger(
      `questions.${selectedQuestionIndex}.props.fieldKey`
    );
    if (response) {
      append({
        type: "binary-radio-selector",
        props: defaultQuestionProps[
          "binary-radio-selector"
        ] as BinaryRadioSelectorProps,
      });
  ...
  const removeQuestion = useCallback(
    (index) => {
      setSelectedQuestionIndex(
        selectedQuestionIndex > 0 ? selectedQuestionIndex - 1 : 0
      );
      remove(index);
