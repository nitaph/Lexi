...
import { ExperimentType, AgentType } from "@models/AppModels";
...
interface ExperimentsProps {
  agents: AgentType[];
  forms: any[]; // Replace 'any' with a proper Form type if available
}

const Experiments: React.FC<ExperimentsProps> = ({ agents, forms }) => {
  ...
  const [editExperiment, setEditExperiment] = useState<ExperimentType | null>(
    null
  );
  ...
  const [openExperimentFormDialog, setOpenExperimentFormDialog] =
    useState(false);
  ...
  const handleStatusChange = (id: string, newStatus: boolean) => {
    setTempExperiments((prev) =>
      prev.map((exp) =>
        exp._id === id ? { ...exp, isActive: newStatus } : exp
      )
    );

    setModifiedExperiments((prev) => {
      const originalStatus = experiments.find((exp) => exp._id === id)
        ?.isActive;
      if (originalStatus === newStatus) {
        ...
        return {
          ...prev,
          [id]: { ...prev[id], id, isActive: newStatus },
        };
      }
    });
  };
  ...
  const handleDeleteExperiment = async () => {
    try {
      ...
      if (editExperiment?._id) {
        await deleteExperiment(editExperiment._id);
        setTempExperiments((prev) =>
          prev.filter((exp) => exp._id !== editExperiment._id)
        );
        setExperiments((prev) =>
          prev.filter((exp) => exp._id !== editExperiment._id)
        );
    ...
    <MainContainerStyled>
      ...
      >
        <Typography variant="body2" fontWeight={500} marginBottom={2}>
          Manage your experiments, attach to them an agent, launch them, share
          with participants, and more.
        </Typography>
        <Box>
          <AddButton
            onClick={() => setOpenExperimentFormDialog(true)}
            size="small"
          >
      ...
      <Dialog open={openDeleteExpDialog} fullWidth maxWidth="md">
        ...
        >
          Deleting the experiment will delete all experiment users and
          conversations. Are you sure you want to delete?
        </WarningMessage>
      </Dialog>
      ...
      >
        ...
        <NextPrevButton
          disabled={Number(expLimit) > experiments.length}
          onClick={() => setPageNumber(String(Number(pageNumber) + 1))}
          size="small"
        >
          <Typography variant="body2" fontWeight={500} color={"floralwhite"}>
