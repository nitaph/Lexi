...
const Admin = () => {
  ...
  useEffectAsync(async () => {
    try {
      const [agentsRes, formsRes] = await Promise.all([
        getAgents(),
        getForms(),
      ]);
      setAgents(agentsRes);
      ...
      openSnackbar("Failed to load data", SnackbarStatus.ERROR);
      setAgents([]);
      setForms([]);
    }
  }, []);
        ...
        >
          <SectionInnerContainer container>
            <Routes>
              <Route
                path="/"
                element={<Navigate to={AdminSections.EXPERIMENTS} replace />}
              />
              <Route
