...
import {
  ...
  updateExperimentDisplaySettings,
...
const Home: React.FC = () => {
  ...
  const startConversation = async () => {
    try {
      ...
      navigate(
        `${Pages.EXPERIMENT_CONVERSATION.replace(
          ":experimentId",
          experimentId
        ).replace(":conversationId", newConversationId)}`
      );
    } catch (err) {
  ...
  return isLoadingPage ? (
    ...
    <StyledContainer>
      ...
      {isEditing ? (
        ...
        >
          {experimentContent?.welcomeContent || (
            <>
              Welcome! This survey is part of a research study conducted by
              Copenhagen Business School (CBS). Your participation will help us
              better understand creativity during human-AI collaboration. The
              survey is anonymous and will take approximately 30 minutes. Your
              responses will be kept confidential and used for academic purposes
              only. Thank you for taking part!
            </>
          )}
        </BodyContent>
      )}
