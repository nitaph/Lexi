...
const FinishConversationDialog = ({
  open,
  setIsOpen,
  questionnaireLink,
  form,
  ...
  const handleDoneSurvey = async (formData) => {
    try {
      await updateConversationMetadata(conversationId, formData, false); // false = post
      if (questionnaireLink) {
        setPage(3);
      }
      await handleDone();
    } catch (error) {
      console.error("Failed to save post-conversation data:", error);
    }
  };
    ...
    <Dialog open={open} maxWidth={"lg"} fullScreen={isMobile && page > 1}>
      {page === 1 ? (
        ...
        <>
          ...
          <DialogContent>
            <DialogContentText color={"black"}>
              Your username is <b>{activeUser.username}</b>, continue with it in
