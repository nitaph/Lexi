      ...
      agent.data.forEach((user) => {
        user.conversations.forEach((conversation) => {
          ...
          if (conversation.metadata.preConversation) {
            Object.keys(conversation.metadata.preConversation).forEach(
              (key) => {
                if (!conversationColFields.has(`pre_${key}`)) {
                  conversationColFields.add(`pre_${key}`);
                  conversationsSheetCol.push({
                    header: `pre_${key}`,
                    key: `pre_${key}`,
                  });
                }
          ...
          if (conversation.metadata.postConversation) {
            Object.keys(conversation.metadata.postConversation).forEach(
              (key) => {
        ...
        user.conversations.forEach((conversation) => {
          const conversationRow = {
            ...
            llmPersonality: conversation.metadata.llmPersonality
              ? `Openness: ${
                  conversation.metadata.llmPersonality.openness * 10
                }, Conscientiousness: ${
                  conversation.metadata.llmPersonality.conscientiousness * 10
                }, Extraversion: ${
                  conversation.metadata.llmPersonality.extraversion * 10
                }, Agreeableness: ${
                  conversation.metadata.llmPersonality.agreeableness * 10
                }, Neuroticism: ${
                  conversation.metadata.llmPersonality.neuroticism * 10
                }`
              : undefined,
            surveySubmittedAt:
              conversation.metadata.postConversation?.submittedAt,
            username: {
          ...
          if (conversation.metadata.preConversation) {
            Object.entries(conversation.metadata.preConversation).forEach(
              ([key, value]) => {
          ...
          if (conversation.metadata.postConversation) {
            Object.entries(conversation.metadata.postConversation).forEach(
              ([key, value]) => {
