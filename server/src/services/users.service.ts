...
class UsersService {
  ...
  createUser = async (
    ...
    if (
      experiment.maxParticipants &&
      experiment.numberOfParticipants + 1 > experiment.maxParticipants
    ) {
  ...
  getActiveUser = async (
    token: string
  ): Promise<{ user: IUser; newToken: string }> => {
    let decoded: any;
  ...
  getUserById = async (userId: string): Promise<IUser> => {
    // Return a full Mongoose document rather than a plain object
  ...
  /**
   * Persist the personalized agent back onto the user document
   */
  updateUserAgent = async (userId: string, agent: IAgent): Promise<void> => {
    await UsersModel.updateOne(
      { _id: new mongoose.Types.ObjectId(userId) },
      { $set: { agent } }
    );
  ...
  /**
   * Save Big-Five trait scores on the user
   */
  updateBigFiveScores = async (
    userId: string,
    scores: {
      openness: number;
    ...
    console.log("🔄 updateBigFiveScores result:", result);
  };
