...
class ConvesationsController {
  message = requestHandler(
    async (req: Request, res: Response) => {
      const {
        message,
        conversationId,
      }: { message: any; conversationId: string } = req.body;
      this.validateMessage(message.content);

      const savedResponse = await conversationsService.message(
        message,
        conversationId
      );
    ...
    (req, res, error) => {
      if (error.code === 403) {
        res.write(
          `data: ${JSON.stringify({
            error: {
              response: { status: 403, data: "Messages Limit Exceeded" },
            },
          })}\n\n`
        );
        res.end();
  ...
  getConversation = requestHandler(async (req: Request, res: Response) => {
    ...
    const conversation =
      await conversationsService.getConversation(conversationId);

    res.status(200).send(conversation);
  });

  updateConversationMetadata = requestHandler(
    async (req: Request, res: Response) => {
