...
class ExperimentsController {
  ...
  getExperimentContent = requestHandler(async (req: Request, res: Response) => {
    ...
    const experiment = await experimentsService.getExperiment(experimentId);
    res.status(200).send({
      content: experiment.displaySettings,
  ...
  createExperiment = requestHandler(async (req: Request, res: Response) => {
    ...
    const savedExperiment =
      await experimentsService.createExperiment(experiment);
  ...
  updateExperimentsStatus = requestHandler(
  ...
  updateExperimentDisplaySetting = requestHandler(
    async (req: Request, res: Response) => {
      const { experimentId, displaySettings } = req.body;
      await experimentsService.updateExperimentDisplaySettings(
        experimentId,
        displaySettings
      );
      res.status(200).send();
  ...
  getRegistrationForm = requestHandler(async (req: Request, res: Response) => {
    ...
    if (experiment.experimentForms?.registration) {
      const form = await formsService.getForm(
        experiment.experimentForms.registration
      );
  ...
  getAllExperimentsByAgentId = requestHandler(
    async (req: Request, res: Response) => {
      const { agentId } = req.query;
      const experiments = await experimentsService.getAllExperimentsByAgentId(
        agentId as string
      );
      res.status(200).send(experiments);
    }
  ...
  getExperimentFeatures = requestHandler(
    async (req: Request, res: Response) => {
      const experimentId = req.params.id as string;
      const experimentFeatures =
        await experimentsService.getExperimentFeatures(experimentId);
